...
async function deployFixture() {
  ...
  const fheAetherContract = (await factory.deploy()) as ERC721Confidential;
  const fheAetherContractAddress = await fheAetherContract.getAddress();

  return { fheAetherContract: fheAetherContract, fheAetherContractAddress: fheAetherContractAddress };
}

describe("ERC721Confidential", function () {
  let signers: Signers;
  let fheAetherContract: ERC721Confidential;
  let fheAetherContractAddress: string;
  ...
  beforeEach(async function () {
    ...
    ({ fheAetherContract, fheAetherContractAddress } = await deployFixture());
  });

  it("encrypted count should be uninitialized after deployment", async function () {
    const encryptedCount = await fheAetherContract.getCount();
